...
testNumericas_VariablesAsociadas <- function(nombre_var){
  ...
  if (resultado$bool){
    ...
    if (p_valor_medias < 0.05){
      ...
      cat(sprintf("La variable %s no influye en el estado vital del paciente, siendo el p-valor de %.7f \n", 
                  nombre_var, p_valor))
    }
  }else{
      # Prueba U de Mann-Whitney
      p_valor <- wilcox.test(resultado$muestra_alive, resultado$muestra_dead)$p.value
      if(p_valor < 0.05){
        lista <- list(nombre_var)
        cat(sprintf("La variable %s influye en el estado vital del paciente, siendo el p-valor de %.7f \n", 
                    nombre_var, p_valor))
